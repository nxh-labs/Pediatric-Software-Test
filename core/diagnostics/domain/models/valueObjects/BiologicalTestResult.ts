  ...
  protected validate(props: Readonly<IBiologicalTestResult>): void {
    if (Guard.isEmpty(props.code).succeeded) {
      throw new EmptyStringError(
        "The BiologicalTestResult code can't be empty. Please provide a valid code."
      );
    }
    if (Guard.isNegative(props.value).succeeded) {
      throw new NegativeValueError(
        "The BiologicalTestResult value can't be an negative value. Please provide a positive value."
      );
    }
    if (Guard.isEmpty(props.unit.unpack()).succeeded) {
      throw new EmptyStringError(
        "The BiologicalTestResult unt can't be empty. Please provide a valid unit."
      );
    }
  }
  static create(
    ...
    try {
      ...
      if (unitCode.isFailure)
        return Result.fail<BiologicalTestResult>(String(unitCode.err));
