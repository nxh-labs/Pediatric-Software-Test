...
export class AnthropometricData extends ValueObject<IAnthropometricData> {
  protected validate(props: Readonly<IAnthropometricData>): void {
    ...
    if (
      (props.height && Guard.isNegative(props.height).succeeded) ||
      (props.length && Guard.isNegative(props.length).succeeded)
    ) {
      throw new NegativeValueError(
        "The patient height or length must be negative."
      );
    }
    if (
      (props.armCircumference &&
        Guard.isNegative(props.armCircumference).succeeded) ||
      (props.headCircumference &&
        Guard.isNegative(props.headCircumference).succeeded)
    ) {
      throw new NegativeValueError(
        "The armCircumference or headCircumference must be negative value."
      );
    }
  }
