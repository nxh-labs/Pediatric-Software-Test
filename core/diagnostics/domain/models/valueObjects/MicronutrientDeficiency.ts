...
export class MicronutrientDeficiency extends ValueObject<IMicronutrientDeficiency> {
  protected validate(props: Readonly<IMicronutrientDeficiency>): void {
    if (
      props.clinicalSignNames.some(
        (value: string) => Guard.isEmpty(value).succeeded
      )
    ) {
      throw new EmptyStringError(
        "The clinical Sign name can't be empty on MicroNutrientDeficiency."
      );
    }
  }
  static create(
    createProps: CreateMicronutrientDeficiency
  ): Result<MicronutrientDeficiency> {
    try {
