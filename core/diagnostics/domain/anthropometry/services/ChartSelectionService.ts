...
export class ChartSelectionService implements IChartSelectionService {
   ...
   private validateChartCondition(chart: AvailableChart, data: AnthropometricVariableObject, standard: GrowthStandard): boolean {
   ...
   async selectChartForIndicator(
      ...
      try {
         ...
         const selectedChart = availableCharts.find((chart) => this.validateChartCondition(chart, data, standard));
