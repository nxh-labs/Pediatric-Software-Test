...
export class ZScoreCalculationService implements IZScoreCalculationService {
    ...
    async calculateZScore(data: AnthropometricVariableObject, indicator: Indicator, chart: GrowthReferenceChart, standard: GrowthStandard): Promise<Result<number>> {
        try {
            ...
            if (!strategy) {
                return handleGrowthIndicatorError(GROWTH_INDICATOR_ERRORS.CALCULATION.STRATEGY_NOT_FOUND.path, `Indicator: ${indicator.getCode()} , Standard: ${standard}`)
            }
            ...
            if (isNaN(zscore)) {
                return handleGrowthIndicatorError(GROWTH_INDICATOR_ERRORS.CALCULATION.INVALID_RESULT.path, `ZScore : ${zscore}, Indicator: ${indicator.getCode()}, Chart: ${chart.getCode()}, Standard: ${standard}`)
            }
